    ...
    <div className="py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="mt-1 text-gray-500">Track your learning and mentoring sessions</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="py-20 text-center">
          ...
          <p className="text-gray-500 mt-2">Book a session from the Explore page to get started!</p>
        </div>
      ) : (
        <div className="space-y-4">
          {sessions.map((session) => {
            ...
            return (
              <div key={session._id} className="p-6 bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md transition-all">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-start gap-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl font-bold shrink-0 ${isMentor ? 'bg-gradient-to-br from-indigo-100 to-blue-100 text-indigo-600' : 'bg-gradient-to-br from-emerald-100 to-teal-100 text-emerald-600'}`}>
                      {otherPerson?.name?.charAt(0) || '?'}
                    </div>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-bold text-gray-900">{otherPerson?.name || 'Unknown'}</h3>
                        <span className={`px-2 py-0.5 text-xs font-semibold rounded-md border ${getStatusBadge(session.status)}`}>
                          {getStatusIcon(session.status)} {session.status}
                        </span>
                        <span className={`px-2 py-0.5 text-xs font-medium rounded-md ${isMentor ? 'bg-indigo-50 text-indigo-700' : 'bg-emerald-50 text-emerald-700'}`}>
                          {role}
                        </span>
                      </div>
                      ...
                      {session.notes && <p className="text-sm text-gray-500 mt-1 italic">"{session.notes}"</p>}
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-2 shrink-0">
                    {/* Accept Button (mentor only, pending sessions) */}
                    {isMentor && session.status === 'pending' && (
                      <button
                        onClick={() => handleStatusUpdate(session._id, 'accepted')}
                        className="px-4 py-2 text-sm font-medium text-white bg-emerald-500 rounded-xl hover:bg-emerald-600 transition"
                      >
                        Accept
                      </button>
                    )}

                    {/* Complete Button (mentor only, accepted sessions) */}
                    ...
                    {/* Join Video Call (accepted sessions) */}
                    {session.status === 'accepted' && session.meetingLink && (
                      <a
                        href={`/video/${session.meetingLink}`}
                        className="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-violet-500 to-purple-500 rounded-xl hover:shadow-lg transition"
                      >
                        🎥 Join Call
                      </a>
                    )}

                    {/* Review Button (completed sessions) */}
                    {session.status === 'completed' && (
                      <button
                        onClick={() => setReviewModal(session)}
                        className="px-4 py-2 text-sm font-medium text-amber-700 bg-amber-100 rounded-xl hover:bg-amber-200 transition"
                      >
                        ⭐ Leave Review
                      </button>
                    )}

                    {/* Cancel Button (pending/accepted sessions) */}
      ...
      {reviewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-6">
            ...
            <div className="mb-6">
              <label className="block mb-1 text-sm font-medium text-gray-700">Comment (optional)</label>
              <textarea
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-primary-600 to-indigo-600 rounded-xl hover:shadow-lg transition"
              >
                Submit Review
              </button>
            </div>
